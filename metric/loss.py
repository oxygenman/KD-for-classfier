...
class RkdDistance(nn.Module):
    def forward(self, student, teacher):
        ...
        return loss
class HKD(nn.Module):
    def forward(self, student, teacher, T):
        with torch.no_grad():
            t_soft=F.softmax(teacher/T,dim=1)
        KD_loss = nn.KLDivLoss()(F.log_softmax(student/ T, dim=1),
                                     t_soft) * (T * T)
        return KD_loss
class NST(nn.Module):
    def forward(self,student, teacher):
        #with torch.no_grad():
        student=F.normalize(student.view(student.shape[0], student.shape[1], -1),dim=1)
        gram_student = student.transpose(1,2).bmm(student)
        assert gram_student.shape[1] == gram_student.shape[2], print("gram_student's shape:{}".format(gram_student.shape))
        with torch.no_grad():
            teacher = F.normalize(teacher.view(teacher.shape[0], teacher.shape[1], -1), dim=1)
            gram_teacher = teacher.transpose(1,2).bmm(teacher)
            assert gram_teacher.shape[1] == gram_student.shape[2], print("gram_teacher's shape: {}".format(gram_teacher.shape))
        loss = (gram_student - gram_teacher).pow(2).mean()
        return loss
